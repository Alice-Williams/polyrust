...
fn require_cpp_system(unit: &mut LanguageUnit<CppImport>, path: &'static str) {
    unit.require_import(cpp_system_group(), CppImport { path, system: true });
}

fn require_cpp_local(unit: &mut LanguageUnit<CppImport>, path: &'static str) {
    unit.require_import(
        cpp_local_group(),
        CppImport {
            path,
...
fn cpp_generated_test_file() -> LanguageSourceFile<CppImport> {
    ...
    let mut body = LanguageUnit::new(CodeDocument::raw_text(RawText::new(
