//! Both original target owners compile independently against subtraction truth.
use super::*;
use std::{fs, path::PathBuf, process::Command};

#[test]
fn wrapping_subtraction_native_values_faults_headers_and_ubsan() {
    let root =
        PathBuf::from(std::env::var_os("TEST_TMPDIR").unwrap()).join("c-wrapping-subtraction");
    ...
    for (name, variant) in [
        ...
        ("reversed-operands", Variant::ReversedOperands),
    ] {
        let directory = root.join(name);
        fs::create_dir_all(&directory).unwrap();
        for owner in chain(Operation::Subtract, variant) {
