...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/stdlib-is-negative-zero"));
    for (target, manifest) in polyrust_stdlib_is_negative_zero_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated stdlib is-negative-zero for rust, typescript, javascript, python, go, java, C++, and C in {}",
        output.display()
    );
}
