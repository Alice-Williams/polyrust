...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/is-fullwidth-code-point"));
    for (target, manifest) in polyrust_is_fullwidth_code_point_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated is-fullwidth-code-point for rust, typescript, javascript, python, go, java, C++, and C in {}",
        output.display()
    );
}
