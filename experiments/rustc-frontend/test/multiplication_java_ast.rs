//! Exact primitive Multiply, original ordered producers and connected typed dataflow.
use super::{MultiplicationInput, MultiplicationWidth};
use crate::java_lower::Reader;
use portable_backend_java::ast::*;
#[path = "multiplication_java_dataflow.rs"]
mod dataflow;
...
pub(super) fn observe<'tcx>(
    ...
    let primitive = match input.width() {
        MultiplicationWidth::I32 => JavaPrimitive::Int,
        MultiplicationWidth::I64 => JavaPrimitive::Long,
    };
    assert_eq!(value.ty, JavaType::primitive(primitive));
    assert_eq!(value.precedence, JavaPrecedence::Multiplicative);
    ...
    *last = *left.clone();
    ...
    assert_ne!(
        ...
        wanted,
        "typed dataflow retains ordered source operand identities"
    );
    eprintln!("MULTIPLICATION_DETACHED\tjava");
    eprintln!("MULTIPLICATION_AST\tjava\t{:?}", input.width());
}
