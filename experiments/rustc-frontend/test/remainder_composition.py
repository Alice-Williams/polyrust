"""Strict native proof of remainder inside the previously admitted operations."""
import json
import os
...
def check(directory, language, zig):
    ...
    conditions = []
    for name, first, second in [("negated", "-1.5", "0.5"), ("absolute", "1.5", "0.5"),
                                ("truncated", "1.0", "-0.0")]:
        conditions += [call(name, "1.5") + " == " + first, call(name, "-2.5") + " == " + second]
    conditions += ["!" + call("nan", "1.5"), call("nan", "Double.NaN" if java else "NAN")]
    negative_zero = call("truncated", "-2.5")
    conditions.append(f"Double.doubleToRawLongBits({negative_zero}) == Long.MIN_VALUE" if java else
                      f"signbit({negative_zero})")
