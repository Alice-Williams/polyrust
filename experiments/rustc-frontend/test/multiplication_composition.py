"""Both call forms, nested/grouped multiplication and an ordinary same-spelled function."""
import os
...
from wrapping_mul_oracle import CASES, inputs, result, signed
...
def check(directory, language, zig):
    ...
    for width, left, right in CASES:
        ...
        wanted.extend([value, value, result(value, 3, width), signed(~value, width)])
        if width == 32:
            wanted.append(left)
    truth = "".join(f"{value}\n" for value in wanted)
