"""Compile-negative executable truncation-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def truncation_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "truncation_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/truncation_contract.rs"],
                rustc_cfgs = ["truncation_contract", "truncation_" + language, "truncation_" + case],
