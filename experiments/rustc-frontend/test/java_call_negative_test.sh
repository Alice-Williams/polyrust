#!/usr/bin/env bash
# Valid Rust outside the registered call subset must never create/replace Java.
set -euo pipefail
adapter="$1"
...
reject() {
    ...
    for existing in no yes; do
        mkdir -p "$work/$name-$existing"
        local output="$work/$name-$existing/Generated.java"
...
reject foreign 'foreign Java direct calls' '
...
reject pointer_signature 'signatures support only i32 and bool' '
fn helper(value: &i32) -> i32 { *value }
pub fn score(value: i32) -> i32 { helper(&value) }'
...
reject recursive 'recursive or excessive-depth Java source calls' '
fn helper(value: i32) -> i32 { helper(value) }
pub fn score(value: i32) -> i32 { helper(value) }'
reject mutual 'recursive or excessive-depth Java source calls' '
...
"$adapter" "$work/control.rs" "$work/control/Generated.java"
test -s "$work/control/Generated.java"
