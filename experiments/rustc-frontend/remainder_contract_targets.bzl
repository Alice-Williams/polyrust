"""Compile-negative executable remainder-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def remainder_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "remainder_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/remainder_contract.rs"],
                rustc_cfgs = ["remainder_contract", "remainder_" + language, "remainder_" + case],
