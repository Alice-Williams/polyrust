...
CASES = {
    "f32": ("pub fn value(v:f32)->f32 {v.abs()}", "Floating absolute value supports only exact f64"),
    "borrow_adjustment": ("pub fn value(v:f64)->f64 {(&v).abs()}", "nongeneric unadjusted by-value"),
    "trait_method": ("trait Fake { fn abs(self)->bool; } impl Fake for bool { fn abs(self)->bool {self} } pub fn value(v:bool)->bool {v.abs()}", "only the standard primitive abs"),
    "trait_qualified": ("trait Fake { fn abs(self)->bool; } impl Fake for bool { fn abs(self)->bool {self} } pub fn value(v:bool)->bool {<bool as Fake>::abs(v)}", "only the standard primitive abs"),
    "inherent_lookalike": ("struct Fake {value:bool} impl Fake {fn abs(self)->bool {self.value}} pub fn value(v:bool)->bool {Fake {value:v}.abs()}", "only the standard primitive abs"),
    ...
    "arithmetic": ("pub fn value(v:f64)->f64 {(v+1.0).abs()}", "only comparison binary operators"),
...
def main():
    root = Path(os.environ["TEST_TMPDIR"]) / "absolute-rejections"
    root.mkdir()
