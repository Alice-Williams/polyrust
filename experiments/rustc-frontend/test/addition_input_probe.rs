...
impl<'tcx> AdditionInput<'tcx> {
    pub(crate) fn probe(
        ...
        let expected_ty = match self.width {
            AdditionWidth::I32 => tcx.types.i32,
            AdditionWidth::I64 => tcx.types.i64,
        };
        ...
        let wrong_width = Self {
            ...
            width: match self.width {
                AdditionWidth::I32 => AdditionWidth::I64,
                AdditionWidth::I64 => AdditionWidth::I32,
            },
        };
        assert!(wrong_width.require_context(tcx, checked).is_err());
        self.require_context(tcx, checked).unwrap();
        eprintln!("ADDITION_INPUT\t{form}\t{:?}", self.width);
        self.source
    }
}
