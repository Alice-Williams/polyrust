//! Target-only arithmetic packages; compiler-owned Rust witnesses are separate.
use super::*;
...
pub(super) fn expression(index: usize, left: JavaExpr, right: JavaExpr) -> JavaExpr {
    use JavaBinaryOperator::{Add, Multiply};
    match index {
        0..=3 => binary(OPERATORS[index], left, right),
        4 => binary(Multiply, binary(Add, left, right.clone()), right),
        5 => binary(
            Add,
            binary(Multiply, left, right),
            literal(0xbff0_0000_0000_0000),
        ),
        _ => unreachable!(),
    }
}
...
pub(super) fn function(index: usize, body: JavaBlock) -> f::Function {
    f::Function {
        ...
        public: true,
        name: f::name(&format!("arithmetic{index}")),
...
pub(super) fn chain() -> Vec<JavaDependencyApi> {
    let declarations = ["leftIdentity", "rightIdentity"]
        ...
        .collect();
    let leaf =
        JavaDependencyApi::from_certificate(f::certify(f::package(501, declarations))).unwrap();
    let middle = owner(502, &leaf, false);
    let root = owner(503, &middle, true);
