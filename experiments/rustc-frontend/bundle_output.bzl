"""Complete C bundle action from an explicit Rust source/metadata provider graph."""
...
def _bundle_impl(ctx):
    ...
    ctx.actions.run_shell(
        ...
        command = """
set -eu
# Remove only Bazel's empty precreated tree, then let the CLI atomically publish
# the actual declared output. A nonempty tree fails closed without modification.
test -d "$2"
rmdir "$2"
"$1" --bundle "$2" "$3"
""",
