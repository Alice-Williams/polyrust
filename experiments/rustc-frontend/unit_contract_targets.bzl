"""Compile-negative executable unit slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def unit_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "unit_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/unit_contract.rs"],
                rustc_cfgs = ["unit_contract", "unit_" + language, "unit_" + case],
