"""Compile-negative executable floating-negation slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def floating_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "floating_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/floating_contract.rs"],
                rustc_cfgs = ["floating_contract", "floating_" + language, "floating_" + case],
