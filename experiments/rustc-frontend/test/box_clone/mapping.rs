//! Executable clone registration with the historical constructor slot retained.
use crate::{
    owned_source::{
        BoxConstructionInput, Builder, OwnedBoxConstruction,
        cloning::{BoxCloneInput, OwnedBoxClone},
    },
...
pub(super) fn bindings<M>(
    mapping: M,
) -> impl Supports<OwnedBoxClone, Mapping = M> + Supports<OwnedBoxConstruction, Mapping = BoxMapping>
where
    M: for<'tcx> Mapping<Capability = OwnedBoxClone, Context<'tcx> = Context<'tcx>, Output = DefId>,
{
    Builder::new()
        .box_clone(mapping)
