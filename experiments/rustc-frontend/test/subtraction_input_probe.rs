...
impl<'tcx> SubtractionInput<'tcx> {
    pub(crate) fn probe(
        ...
        let expected_ty = match self.width {
            SubtractionWidth::I32 => tcx.types.i32,
            SubtractionWidth::I64 => tcx.types.i64,
        };
        ...
        assert_eq!(tcx.item_name(definition).as_str(), "wrapping_sub");
        ...
        let wrong_width = Self {
            ...
            width: match self.width {
                SubtractionWidth::I32 => SubtractionWidth::I64,
                SubtractionWidth::I64 => SubtractionWidth::I32,
            },
        };
        assert!(wrong_width.require_context(tcx, checked).is_err());
        self.require_context(tcx, checked).unwrap();
        eprintln!("SUBTRACTION_INPUT\t{form}\t{:?}", self.width);
        self.source
    }
}
