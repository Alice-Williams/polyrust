"""Compile-negative checks for the executable lazy Boolean slot."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def short_circuit_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "lazy_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/short_circuit_contract.rs"],
                rustc_cfgs = ["lazy_contract", "lazy_" + language, "lazy_" + case],
