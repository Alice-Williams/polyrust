...
authoritative tests run in the Linux DevContainer. Each 32-bit entry corpus contains the
...
The i64 package proof uses `i64_values.rs` and its actual dependency
`i64_leaf.rs`, generated by `generated_java_i64` and `generated_c_i64`.
`i64_native_test` checks 21 x 21 x 2 x 20 exact integer/Boolean results against
both `rust_i64_values` and an independent integer oracle. Libraries and consumers
compile separately; a deliberate narrowing mutation must fail the oracle.
Test-only instrumentation checks exact operand/call traces for all six wide
comparisons, a condition and lazy imported arguments. Reordered comparison
mutants must preserve results but fail the trace oracle.
`i64_rejection_test` covers 44 atomic source rejections and the Java 255/256
parameter-slot boundary. `literal_private_*` compile contracts protect checked
literal construction. Local export aliases are covered; foreign re-exporting is
explicitly still pending in M35-03A-03. See the
[i64 task](../../../../plan/tasks/M35-03A-02C-i64-values.md) for completion status.

The Boolean-negation public-package fixture is generated by
`generated_java_boolean_negation` and `generated_c_boolean_negation`, with native
reference `rust_boolean_negation` and `boolean_negation_native_test` (8,204 x 13).
It exercises literals, nested Not, scalar comparison operands, field/shared
reads, shadowing, branch conditions and calls. It also checks exact artifact and
