...
def constant_ast_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        compiler_adapter(
            name = "constant_" + language + "_ast_probe",
            crate_root = root,
            rustc_cfg = "constant_ast_probe",
            directory_publisher = ":directory_publisher" if language == "c" else None,
            srcs = sources + ["test/constant_" + language + "_ast.rs"],
            deps = [
    ...
    inputs = [":constant_c_ast_probe", ":adapter", ":constant_java_ast_probe", ":java_adapter", "fixtures/constant_values.rs", "fixtures/constant_leaf.rs"]
    sh_test(
        name = name,
        size = "medium",
        srcs = ["test/java_source_native.sh"],
        args = ["$(rootpath test/constant_ast.py)"] + ["$(rootpath " + item + ")" for item in inputs],
        data = ["test/constant_ast.py"] + inputs,
    )
