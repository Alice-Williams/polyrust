...
readonly generator="$(find "${runfiles}" -path '*/examples/real-world/strip-bom/generate' -print -quit)"
readonly compare="$(find "${runfiles}" -path '*/examples/real-world/strip-bom/compare.mjs' -print -quit)"
readonly upstream="$(find "${runfiles}" -path '*/third_party/strip-bom/upstream.js' -print -quit)"
