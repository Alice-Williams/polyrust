"""Compile-negative checks for the executable constant Boolean slot."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def constant_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "constant_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/constant_contract.rs"],
                rustc_cfgs = ["constant_contract", "constant_" + language, "constant_" + case],
