...
pub(crate) struct WrappingSubtraction;
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum SubtractionWidth {
    I32,
    I64,
}

#[cfg(subtraction_ast_probe)]
#[path = "../../test/subtraction_input_probe.rs"]
mod probe;

pub(crate) struct SubtractionInput<'tcx> {
    source: &'tcx hir::Expr<'tcx>,
    left: &'tcx hir::Expr<'tcx>,
    right: &'tcx hir::Expr<'tcx>,
    definition: DefId,
    width: SubtractionWidth,
}
impl Capability for WrappingSubtraction {
    type Input<'tcx> = SubtractionInput<'tcx>;
}

impl<'tcx> SubtractionInput<'tcx> {
    pub(crate) fn discover(
        ...
        let hir::Node::Expr(canonical) = tcx.hir_node(expression.hir_id) else {
            return Err("wrapping subtraction requires a canonical HIR expression".into());
        };
        if !std::ptr::eq(canonical, expression)
            || !std::ptr::eq(checked, tcx.typeck(expression.hir_id.owner.def_id))
        {
            return Err(
                "wrapping subtraction requires its original checked function and HIR node".into(),
            );
        }
        let (definition, arguments, left, right) = match canonical.kind {
            hir::ExprKind::MethodCall(_, left, [right], _) => (
                checked
                    ...
                    .ok_or("wrapping subtraction method is unresolved")?,
            ...
            hir::ExprKind::Call(callee, [left, right]) => {
                ...
                let ty::FnDef(typed, arguments) = *checked.expr_ty(callee).kind() else {
                    return Err("wrapping subtraction requires a direct associated function".into());
                };
                if typed != resolved || !checked.expr_adjustments(callee).is_empty() {
                    return Err(
                        "wrapping subtraction associated identity or adjustment differs".into(),
                    );
                }
        ...
        if tcx.item_name(definition).as_str() != "wrapping_sub" {
            return Ok(None);
        }
        let core = tcx
            .lang_items()
            ...
            .ok_or("wrapping subtraction requires the standard core language items")?
            .krate;
        if definition.is_local()
            || definition.krate != core
            || tcx.def_kind(definition) != DefKind::AssocFn
        {
            return Err("only the standard primitive wrapping_sub method is supported".into());
        }
        let implementation = tcx
            .inherent_impl_of_assoc(definition)
            .ok_or("wrapping subtraction requires an inherent primitive implementation")?;
        ...
        {
            return Err(
                "wrapping subtraction requires nongeneric unadjusted by-value operands".into(),
            );
        }
        let owner = tcx
            ...
            .map_err(|_| "wrapping subtraction primitive owner normalization failed")?;
        let width = match owner.kind() {
            ty::Int(ty::IntTy::I32) => SubtractionWidth::I32,
            ty::Int(ty::IntTy::I64) => SubtractionWidth::I64,
            _ => return Err("wrapping subtraction supports only exact i32 and i64".into()),
        };
        if [canonical, left, right]
            .into_iter()
            .any(|node| checked.expr_ty(node) != owner)
        {
            return Err("wrapping subtraction operand/result widths differ".into());
        }
        let signature = tcx
            .try_normalize_erasing_regions(
                ty::TypingEnv::fully_monomorphized(),
            ...
            .map_err(|_| "wrapping subtraction signature normalization failed")?;
        if !signature.bound_vars().is_empty() {
            return Err("wrapping subtraction cannot instantiate bound signature variables".into());
        }
        ...
        {
            return Err("wrapping subtraction requires the exact safe primitive signature".into());
        }
        Ok(Some(Self {
            source: canonical,
            left,
    ...
    pub(crate) fn require_context(
        ...
        let current = Self::discover(tcx, checked, self.source)?
            .ok_or("wrapping subtraction witness no longer names a built-in operation")?;
        ...
        {
            return Err("wrapping subtraction witness identity differs".into());
        }
        Ok(())
    }
    ...
    pub(crate) fn width(&self) -> SubtractionWidth {
        self.width
    }
}
