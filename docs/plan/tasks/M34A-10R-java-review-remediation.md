...
- Focused Java tests, every historical Java target, source/template policies,
  linters, the complete tracked-rule graph, and `//:release_gate` pass in the
  Linux container. Bazel action/test caches remain enabled under M16A.
...
plus eight doctests. The subsequent immutable `fb3803e` review identified five
resource families: oversized literals, parameter slots, encoded names and
descriptors, method code length, and aggregate class capacity. The accepted
