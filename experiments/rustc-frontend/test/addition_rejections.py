...
CASES = {
    "i8": ("pub fn value()->bool { 1i8.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "i16": ("pub fn value()->bool { 1i16.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "i128": ("pub fn value()->bool { 1i128.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "isize": ("pub fn value()->bool { 1isize.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "u32": ("pub fn value()->bool { 1u32.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "u64": ("pub fn value()->bool { 1u64.wrapping_add(2)==3 }", "supports only exact i32 and i64"),
    "borrowed32": ("pub fn value(a:i32,b:i32)->i32 { (&a).wrapping_add(b) }", "unadjusted by-value operands"),
    "borrowed64": ("pub fn value(a:i64,b:i64)->i64 { (&a).wrapping_add(b) }", "unadjusted by-value operands"),
    "cast": ("pub fn value(a:i64,b:i32)->i32 { (a as i32).wrapping_add(b) }", "signed widening supports only an unadjusted i32 operand cast to i64"),
    ...
    "trait": ("struct Fake {v:i32} trait Add {fn wrapping_add(self,b:i32)->i32;} impl Add for Fake {fn wrapping_add(self,_b:i32)->i32 {self.v}} pub fn value(a:i32,b:i32)->i32 {Fake{v:a}.wrapping_add(b)}", "only the standard primitive wrapping_add"),
    "inherent": ("struct Fake {v:i32} impl Fake {fn wrapping_add(self,_b:i32)->i32 {self.v}} pub fn value(a:i32,b:i32)->i32 {Fake{v:a}.wrapping_add(b)}", "only the standard primitive wrapping_add"),
}


def main():
    root = Path(os.environ["TEST_TMPDIR"]) / "addition-rejections"
    root.mkdir()
    ...
    print(f"{4 * len(CASES)} atomic unsupported wrapping-addition rejections")
