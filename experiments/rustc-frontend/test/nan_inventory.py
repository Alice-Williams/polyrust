...
from nan_oracle import OPERATIONS, LITERALS
...
def inspect(java_dir, c_dir):
    ...
    names = {leaf: ["is_nan", "classify"], middle: ["relay", "classify"],
    ...
    for owner, items in names.items():
        for name in items:
            ...
            assert declaration["result"] == ("f64" if name in ["is_nan", "relay"] else "bool")
    ...
    marker, = [d for d in functions.values()
               if [s.strip() for s in d["documentation"]] == ["Trace N."]]
