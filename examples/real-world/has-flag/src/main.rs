...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/has-flag"));
    for (target, manifest) in polyrust_has_flag_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated has-flag for rust, typescript, javascript, python, go, java, C++, and C in {}",
        output.display()
    );
}
