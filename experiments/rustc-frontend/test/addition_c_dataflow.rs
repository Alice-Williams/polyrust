...
fn sum(reader: &Reader<'_>, width: CScalarType, left: CValue, right: CValue) -> CValue {
    ...
    let guard = e
        ...
        .unwrap();
    let nonnegative = e.numeric_conversion(width, unsigned_sum.clone()).unwrap();
    let negative = e
        .binary(
            ...
            e.numeric_conversion(
                width,
                e.unary(CUnaryOperator::BitNot, unsigned_sum).unwrap(),
            )
            .unwrap(),
        )
        .unwrap();
