"""Compile-negative executable wrapping-negation slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def wrapping_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "wrapping_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/wrapping_contract.rs"],
                rustc_cfgs = ["wrapping_contract", "wrapping_" + language, "wrapping_" + case],
