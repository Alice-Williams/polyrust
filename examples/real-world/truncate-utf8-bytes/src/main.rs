...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/truncate-utf8-bytes"));
    for (target, manifest) in polyrust_truncate_utf8_bytes_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated truncate-utf8-bytes for rust, typescript, javascript, python, go, java, C++, and C in {}",
        output.display()
    );
}
