...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/split-on-first"));
    for (target, manifest) in polyrust_split_on_first_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated split-on-first for rust, typescript, javascript, python, go, java, C++, and C in {}",
        output.display()
    );
}
