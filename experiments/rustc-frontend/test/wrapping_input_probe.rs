...
impl<'tcx> WrappingInput<'tcx> {
    pub(crate) fn probe(&self, tcx: TyCtxt<'tcx>, checked: &TypeckResults<'tcx>) {
        ...
        assert_eq!(tcx.item_name(definition).as_str(), "wrapping_neg");
        ...
        let expected = match owner.kind() {
            ...
            _ => panic!("exact primitive owner"),
        };
        assert_eq!(expected, self.width);
        assert_eq!(checked.expr_ty(self.receiver), owner);
        assert_eq!(checked.expr_ty(self.source), owner);
        let signature = tcx
            .try_normalize_erasing_regions(
                ty::TypingEnv::fully_monomorphized(),
        ...
        assert_eq!(signature.output(), owner);
        ...
        eprintln!("WRAPPING_INPUT\t{form}\t{expected:?}");
    }
}
