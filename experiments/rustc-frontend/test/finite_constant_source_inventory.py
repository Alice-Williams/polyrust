...
from finite_constant_source_truth import CONSTANTS, READS, reject
...
def inspect(c_dir, j_dir, mixed):
    ...
    expected = {(producer, "value", name) for name in [*CONSTANTS, "TENTH_ALIAS"]}
    expected |= {(second, "value", "TENTH"), (second, "value", "SAME_VALUE"), (middle, "type", "nested"),
                 (nested, "type", "cycle"), (nested, "value", "AGAIN")}
    expected |= {(middle, "value", name) for name in [*CONSTANTS, "OTHER_TENTH", "OTHER_SAME"]}
    if mixed:
        expected |= {(root, "value", name) for name in [
            "NEGATIVE_ZERO", "POSITIVE_ZERO", "TENTH", "RENAMED", "OWN",
    ...
    assert binding(producer, "TENTH_ALIAS") == binding(nested, "AGAIN") == binding(producer, "TENTH")
    for name in CONSTANTS:
        assert binding(producer, name) == binding(middle, name)
    assert binding(middle, "OTHER_TENTH") == binding(second, "TENTH") != binding(producer, "TENTH")
    assert binding(middle, "OTHER_SAME") == binding(second, "SAME_VALUE") != binding(second, "TENTH")
    assert binding(producer, "SAME_VALUE") != binding(producer, "TENTH")
    values = {binding(producer, name): value for name, value in CONSTANTS.items()}
    values[binding(second, "TENTH")] = CONSTANTS["TENTH"]
    values[binding(second, "SAME_VALUE")] = CONSTANTS["TENTH"]
    if mixed:
        values[binding(root, "OWN")] = CONSTANTS["INDEXED"]
    ...
    identity = binding(producer, "POSITIVE_ZERO")
    ...
    for name, description in {
        "constants": "Original finite constants: exact bits and compiler-evaluated expressions.",
        "second": "Independent producer with a numerically equal constant.",
        "middle": "Alias-only façade; constants retain their original producer.",
        "root": "Mixed finite-constant owner with private, local and inherent reads.",
    }.items():
        if name in owners:
            docs[owners[name]] = [description]
    docs[nested] = ["Aliases preserve original module identity, including a finite module cycle."]
    docs[binding(producer, "POSITIVE_ZERO")] = ["Positive zero; changing its sign must invalidate every dependent."]
    docs[binding(producer, "NEGATIVE_ZERO")] = ["Negative zero is a distinct constant representation."]
    docs[binding(producer, "INDEXED")] = ["Constant evaluation may use arrays without admitting runtime array indexing."]
    if mixed:
        docs[binding(root, "OWN")] = ["Root-owned constant, not an alias of a dependency."]
    for owner in c:
