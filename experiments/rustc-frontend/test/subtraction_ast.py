...
import subprocess
import sys
from subtraction_composition import check
...
def main():
    ...
    work = Path(os.environ["TEST_TMPDIR"]) / "subtraction-ast"
    work.mkdir()
    for language, probe, production in [("c", c_probe, c), ("java", java_probe, java)]:
        ...
        for label, adapter in [("probe", probe), ("production", production)]:
            ...
            observed = Counter(line for line in result.stderr.splitlines() if line.startswith("SUBTRACTION_"))
            wanted = Counter({f"SUBTRACTION_INPUT\t{form}\t{width}": 4 if form == "method" else 1
                              for form in ["method", "associated"] for width in ["I32", "I64"]})
            wanted.update({f"SUBTRACTION_AST\t{language}\t{width}": 5 for width in ["I32", "I64"]})
            wanted.update({"SUBTRACTION_DETACHED\t" + language: 10})
