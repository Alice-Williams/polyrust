"""Three original source owners; exact Rust/C/Java sign/category and call proof."""
import os
from pathlib import Path
import subprocess
import sys
from floating_inventory import inspect
from floating_oracle import VALUES, OPERATIONS, LITERALS, expected, traces, observed
from floating_consumers import c_consumer, java_consumer
from floating_mutations import mutate
from short_circuit_mutations import instrument
from floating_examples import export
...
def main():
    ...
    work_root = Path(os.environ["TEST_TMPDIR"]) / "floating-native"
    work_root.mkdir()
    ...
    for is_java, directory, owners in [(True, java_dir, java), (False, c_dir, c)]:
        ...
        for variant in ["plain", "traced", "no_negation", "zero_minus", "dropped", "duplicated"]:
            ...
            for owner in order:
                ...
                if owner == root and variant != "plain":
                    text = instrument(text, {"G": marker}, is_java)
                    if variant != "traced":
                        target = "direct" if variant in ["no_negation", "zero_minus"] else "local"
            ...
            for result in results:
                ...
                if variant in ["no_negation", "zero_minus"]:
                    original = list(wanted)
                    for index, bits in enumerate(VALUES):
                        if variant == "no_negation" or bits == 0:
                            wanted[2 + index * len(OPERATIONS)] = observed(bits)
                    assert wanted != original
                assert result.stdout.splitlines() == wanted, (is_java, variant, "exact sign oracle")
    ...
    print(f"{len(truth.splitlines())} exact Rust/C/Java sign/category results; three crates, "
