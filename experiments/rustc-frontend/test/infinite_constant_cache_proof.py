...
BUNDLES = ["generated_" + lang + "_infinite_constant_" + owner
           for lang in ["c", "java"] for owner in ["middle", "root"]]
METADATA = ["infinite_constant_" + owner + "_metadata" for owner in OWNERS]
TARGETS = [PREFIX + name for name in BUNDLES + METADATA]
NATIVE = PREFIX + "infinite_constant_source_native_test"
...
def main():
    ...
    evidence = []
    with tempfile.TemporaryDirectory(prefix="polyrust-infinite-constant-cache-") as directory:
        work = Path(directory)
        with tarfile.open(archive) as source:
            source.extractall(work, filter="data")
        fixture = work / "experiments/rustc-frontend/fixtures/infinite_constant_data.rs"
        oracle = work / "experiments/rustc-frontend/test/infinite_constant_source_truth.py"
        original, original_oracle = fixture.read_bytes(), oracle.read_bytes()
        old_source = b"pub const BITS_POSITIVE: f64 = f64::from_bits(0x7ff0_0000_0000_0000);"
        old_truth = b'CONSTANTS["BITS_POSITIVE"] = INFINITY'
        ...
        for label in ["infinity-sign"]:
            try:
                fixture.write_bytes(original.replace(old_source, b"pub const BITS_POSITIVE: f64 = f64::from_bits(0xfff0_0000_0000_0000);"))
                actions = bazel(label, "build", TARGETS, actions=True)
                labels = {item["target"] for item in actions}
                independent = PREFIX + "infinite_constant_second_metadata"
                ...
                for owner in OWNERS:
                    name = "libinfinite_constant_" + owner + "_metadata.rmeta"
                ...
                oracle.write_bytes(original_oracle.replace(old_truth, b'CONSTANTS["BITS_POSITIVE"] = SIGN | INFINITY'))
        ...
        print("Infinity sign: affected metadata/bundles invalidated, independent producer retained, "
