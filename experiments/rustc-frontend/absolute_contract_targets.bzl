"""Compile-negative executable absolute-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def absolute_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "absolute_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/absolute_contract.rs"],
                rustc_cfgs = ["absolute_contract", "absolute_" + language, "absolute_" + case],
