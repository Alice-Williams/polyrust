...
fn main() {
    let output = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("generated/models-and-validation"));
    for (target, manifest) in polyrust_models_and_validation_example::manifests() {
        for file in manifest.files() {
            write_file(&output, target, file.path(), file.contents());
        }
    }
    println!(
        "generated rust, typescript, javascript, python, go, and java in {}",
        output.display()
    );
}
