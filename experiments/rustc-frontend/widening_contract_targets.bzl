"""Compile-negative executable widening-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def widening_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "widening_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/widening_contract.rs"],
                rustc_cfgs = ["widening_contract", "widening_" + language, "widening_" + case],
