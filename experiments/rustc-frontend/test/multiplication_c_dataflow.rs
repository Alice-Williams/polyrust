...
use rustc_middle::ty;
#[path = "multiplication_source_ast.rs"]
mod source;

fn product(reader: &Reader<'_>, width: CScalarType, left: CValue, right: CValue) -> CValue {
    ...
    let guard = e
        .binary(
            ...
            unsigned_product.clone(),
            e.literal(CLiteral::Unsigned(max)).unwrap(),
        )
        .unwrap();
    let nonnegative = e
        .numeric_conversion(width, unsigned_product.clone())
        .unwrap();
    let negative = e
        .binary(
            ...
            e.numeric_conversion(
                width,
                e.unary(CUnaryOperator::BitNot, unsigned_product).unwrap(),
            )
            .unwrap(),
        )
        .unwrap();
...
pub(super) fn expected<'tcx>(reader: &Reader<'tcx>, value: &'tcx hir::Expr<'tcx>) -> CValue {
    ...
    match value.kind {
        ...
        _ => panic!("closed multiplication fixture grammar"),
    }
}
...
pub(super) fn expanded(
    ...
    match value.kind() {
        ...
        _ => panic!("closed multiplication target grammar"),
    }
}
