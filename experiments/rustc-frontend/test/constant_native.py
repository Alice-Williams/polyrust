...
from constant_inventory import inspect
from constant_oracle import CASES, expected
...
def main():
    ...
    truth = expected()
    assert len(truth.splitlines()) == 38
    assert run([reference]).stdout == truth
    work_root = Path(os.environ["TEST_TMPDIR"]) / "constant-native"
    work_root.mkdir()
    ...
    print("38 exact Rust/C/Java constant results, two crates, GCC/Zig O0/O2; wrong-value faults detected")
