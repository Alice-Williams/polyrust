...
fn type_negative_file() -> LanguageSourceFile<PythonImport> {
    ...
    let mut body = LanguageUnit::new(CodeDocument::raw_text(RawText::new(TYPE_NEGATIVE_BODY)));
    body.require_import(
        local_group(),
        PythonImport::From {
            module: "generated_polyrust.runtime",
...
impl<'a> Generator<'a> {
    ...
    fn require_declaration_imports(
        &self,
        unit: &mut LanguageUnit<PythonImport>,
...
fn require_annotations(unit: &mut LanguageUnit<PythonImport>) {
    unit.require_import(future_group(), PythonImport::Future("annotations"));
}

fn require_callable_imports(unit: &mut LanguageUnit<PythonImport>) {
    require_annotations(unit);
    require_from(unit, standard_group(), "typing", "cast");
    require_from(unit, local_group(), ".runtime", "PolyResult");
}

fn require_type(unit: &mut LanguageUnit<PythonImport>, ty: &TypeRef) {
    match ty {
