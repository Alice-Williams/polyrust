...
export const indexOfLiteralScalars = (source, needle) => {
...
export const sliceScalars = (source, rawStart, rawEnd) => {
    const scalars = Array.from(source);
    const length = BigInt(scalars.length);
    const clamp = (value) => value < 0n ? 0n : value > length ? length : value;
...
export const truncateUtf8Bytes = (source, budget) => {
    let bytes = 0;
    let end = 0;
    for (const scalar of source) {
        ...
        if (bytes > budget)
            return source.slice(0, end - scalar.length);
    }
    return source;
};
    ...
    intrinsic(name, values) {
        ...
        switch (name) {
            ...
            case "float_abs": {
                ...
                view.setFloat64(0, a);
                view.setBigUint64(0, view.getBigUint64(0) & 0x7fffffffffffffffn);
            ...
            case "list_index_of": {
                const index = a.findIndex((item) => this.equal(item, b));
                return ok(index < 0 ? none() : some(BigInt(index)));
            }
