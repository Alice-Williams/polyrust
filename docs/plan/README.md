...
- [M35-03A — Runtime-free C/Java parity and cleanup](tasks/M35-03A-runtime-free-parity.md) — inventory/guards and bounded Boolean/i64/bitwise/bool-i32-i64-constant steps complete, including authenticated cross-crate aliases, cache proof and unit-result C/Java compiler integration; wrapping-negation target foundation and checked compiler integration are complete; binary64 finite-literal/target foundations and checked Rust C/Java value/comparison integration are complete, with exact native/AST/atomic proof and actual exported examples; built-in binary64 negation target proof and checked compiler integration are complete with 804/804 release tests and clean independent reviews; standard inherent f64::is_nan (M35-03A-02K) is complete with 822/822 release tests and clean independent reviews; binary64 absolute value (M35-03A-02L) target and checked source integration are complete with 840/840 release tests, native/AST/atomic/ordinary-call proof, actual examples and independent reviews; binary64 truncation (M35-03A-02M) has a completed C target foundation with 840/840 tests, native stack/link/value/trace proof and clean independent reviews; the Java target foundation is also complete with 840/840 release tests, 376 Java tests and two clean independent reviews; compiler integration remains; remaining functionality must be implemented before legacy removal
