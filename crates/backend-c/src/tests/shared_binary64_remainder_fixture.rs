...
pub(super) fn build(
    ...
    let header = registry
        .register_file(CFileKey {
            path: RelativeOutputPath::new(format!("polyrust_remainder_{crate_id}.h")).unwrap(),
            role: CFileRole::GeneratedPublicHeader,
        })
        .unwrap();
    let implementation = registry
        .register_file(CFileKey {
            path: RelativeOutputPath::new(format!("polyrust_remainder_{crate_id}.c")).unwrap(),
    ...
    for index in 0..1 {
        ...
        let function = registry
            .register_function(
                &header,
                CDeclarationKey {
                    name: CIdentifier::new(&format!("poly_remainder_{crate_id}_{index}")).unwrap(),
        ...
        let scope = registry
            ...
            .unwrap();
        locals.push(matches!(mode, Body::Remainder).then(|| {
            [
                registry
