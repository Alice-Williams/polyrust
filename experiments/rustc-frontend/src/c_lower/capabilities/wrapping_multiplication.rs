//! Unsigned modulo product and guarded signed reconstruction; no signed overflow.
use super::{Mapping, MultiplicationInput, MultiplicationWidth, WrappingMultiplication};
use crate::c_lower::{Reader, Result, c};
use portable_backend_c::ast::*;

#[derive(Clone, Copy)]
pub(crate) struct CWrappingMultiplication;
impl Mapping for CWrappingMultiplication {
    ...
    fn lower<'tcx>(
        ...
        if left.ty().kind() != &CObjectTypeKind::Scalar(signed) || right.ty() != left.ty() {
            return Err(
                "wrapping multiplication C representation differs from checked width".into(),
            );
        }
        ...
        let product = c(reader
            .expressions()
            .binary(CBinaryOperator::Multiply, left, right))?;
        let product = reader.materialize(product)?;
        let e = reader.expressions();
        let maximum = c(e.literal(CLiteral::Unsigned(maximum)))?;
        let comparison = c(e.binary(CBinaryOperator::LessEqual, product.clone(), maximum))?;
        let condition = c(e.numeric_conversion(CScalarType::Bool, comparison))?;
        let positive = c(e.numeric_conversion(signed, product.clone()))?;
        let complement = c(e.unary(CUnaryOperator::BitNot, product))?;
        ...
        #[cfg(multiplication_ast_probe)]
        super::multiplication_ast::observe(reader, &input, &result, start, middle, end_operands);
        Ok(result)
    }
}
