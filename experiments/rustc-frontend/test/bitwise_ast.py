...
def main():
    ...
    root = Path(os.environ["TEST_TMPDIR"]) / "bitwise-ast"
    root.mkdir()
    ...
    for language, probe, production in [("c", c_probe, c), ("java", java_probe, java)]:
        ...
        for label, adapter in [("probe", probe), ("production", production)]:
            ...
            markers = [line for line in result.stderr.splitlines() if line == "BITWISE_AST\t" + language]
            assert len(markers) == (28 if label == "probe" else 0), (language, label, result.stderr)
            results.append(contents(work))
        assert results[0] == results[1], (language, "probe changed generated artifacts")
    print("28 bitwise AST projections per target preserve operators, types, promotions/precedence and exact production bytes")
