"""Compile-negative executable subtraction-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def subtraction_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "subtraction_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/subtraction_contract.rs"],
                rustc_cfgs = ["subtraction_contract", "subtraction_" + language, "subtraction_" + case],
