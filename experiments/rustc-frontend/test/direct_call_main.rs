...
impl Callbacks for Probe {
    fn after_analysis<'tcx>(&mut self, _: &interface::Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        ...
        for _ in 0..3 {
            let (registry, source) = c_lower::check(tcx, root).unwrap();
            direct_call_assertions::check(tcx, &source);
...
fn main() -> std::process::ExitCode {
    ...
    let compiler_args = vec![
        ...
        args[1].clone(),
        "--crate-type=lib".into(),
        "--crate-name=poly_direct_call_probe".into(),
        "--edition=2024".into(),
        "-Funsafe-code".into(),
        "-Cpanic=abort".into(),
    ];
    ...
    println!("resolved calls, exact prelude/branch order, private linkage and determinism passed");
