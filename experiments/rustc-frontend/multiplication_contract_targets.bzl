"""Compile-negative executable multiplication-value slot and checked-input boundaries."""

load(":defs.bzl", "compiler_adapter_compile_fail_test")

def multiplication_contract_targets(name, c_sources):
    ...
    for language in ["c", "java"]:
        ...
        for case, error in [
            ...
            target = "multiplication_" + language + "_" + case + "_test"
            tests.append(":" + target)
            compiler_adapter_compile_fail_test(
                name = target,
                crate_root = root,
                srcs = sources + ["test/multiplication_contract.rs"],
                rustc_cfgs = ["multiplication_contract", "multiplication_" + language, "multiplication_" + case],
