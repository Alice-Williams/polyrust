...
CASES = {
    ...
    "unit_parameter": ("fn action(_: ()) {}", "action(());", "direct-call signatures support only"),
    "tuple_result": ("fn action(v: i32) -> (i32, i32) { (v, v) }", "let pair = action(value);", "direct-call signatures support only"),
    ...
    "never_result": ("fn action(_value: i32) -> ! { loop {} }", "action(value);", "direct-call signatures support only"),
