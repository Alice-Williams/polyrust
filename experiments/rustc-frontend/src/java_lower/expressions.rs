    ...
    pub(super) fn expr(&mut self, value: &'tcx hir::Expr<'tcx>) -> Result<Value> {
        self.bounded(|reader| {
            ...
            let result = if let Some(input) =
                WideningInput::discover(reader.tcx, reader.checked, value)?
            {
                Supports::<SignedWidening>::mapping(&reader.mappings).lower(reader, input)?
                ...
                SubtractionInput::discover(reader.tcx, reader.checked, value)?
            {
                ...
                MultiplicationInput::discover(reader.tcx, reader.checked, value)?
            {
                Supports::<WrappingMultiplication>::mapping(&reader.mappings)
                    .lower(reader, input)?
