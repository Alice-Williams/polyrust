...
fn require_c_system(unit: &mut LanguageUnit<CImport>, path: &'static str) {
    unit.require_import(c_system_group(), CImport { path, system: true });
}

fn require_c_local(unit: &mut LanguageUnit<CImport>, path: &'static str) {
    unit.require_import(
        c_local_group(),
        CImport {
            path,
...
fn c_runtime_header_file() -> LanguageSourceFile<CImport> {
    ...
    file.set_preamble(LanguageUnit::new(guarded_preamble(
        GUARD,
    ...
    file.set_body(body);
    file.set_epilogue(LanguageUnit::new(guarded_epilogue(GUARD)));
    file
}
...
fn c_generated_header_file(generator: &Generator<'_>) -> LanguageSourceFile<CImport> {
    ...
    let mut body = LanguageUnit::new(CodeDocument::raw_text(RawText::new(generator.header())));
    require_c_local(&mut body, "runtime.h");
    file.set_body(body);
    file.set_epilogue(LanguageUnit::new(guarded_epilogue(&guard)));
    file
...
fn c_generated_source_file(
    ...
    let mut body = LanguageUnit::new(CodeDocument::raw_text(RawText::new(generator.source()?)));
    require_c_system(&mut body, "string.h");
...
fn c_generated_test_file(
    ...
    let mut body = LanguageUnit::new(CodeDocument::raw_text(RawText::new(generator.tests()?)));
    require_c_system(&mut body, "string.h");
    require_c_local(&mut body, "generated.h");
    file.set_body(body);
    Ok(file)
