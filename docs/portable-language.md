...
| Float | `neg`, `abs`, `add`, `sub`, `mul`, `div`, `rem_trunc`, `trunc`, `is_nan`, `is_negative_zero` | IEEE 754 binary64, including NaN/infinities/negative zero; `abs` clears only the sign bit and preserves every exponent/fraction bit; remainder uses truncating quotient; `trunc` rounds toward zero; `is_nan` recognizes every NaN payload/sign and no other value; `is_negative_zero` recognizes only raw bits `0x8000000000000000` |
