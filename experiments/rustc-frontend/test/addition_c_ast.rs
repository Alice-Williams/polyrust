...
use super::{AdditionInput, AdditionWidth};
use crate::c_lower::Reader;
use portable_backend_c::ast::*;
#[path = "addition_c_dataflow.rs"]
mod dataflow;
...
pub(super) fn observe<'tcx>(
    ...
    let (signed, unsigned) = match input.width() {
        AdditionWidth::I32 => (CScalarType::I32, CScalarType::U32),
        AdditionWidth::I64 => (CScalarType::I64, CScalarType::U64),
    };
    assert_eq!(value.ty().kind(), &CObjectTypeKind::Scalar(signed));
    assert_eq!(reader.prelude.len(), end + 1, "one unsigned sum temporary");
    ...
    let CStatementKind::Declare(sum) = reader.prelude[end].kind() else {
        panic!("sum")
    };
    let CInitializerKind::Expression(sum_value) = sum.initializer().unwrap().kind() else {
        panic!("sum initializer")
    };
    assert_eq!(sum_value.ty().kind(), &CObjectTypeKind::Scalar(unsigned));
    assert_eq!(
        *sum_value,
        e.binary(
            CBinaryOperator::Add,
    ...
    eprintln!("ADDITION_DETACHED\tc");
    eprintln!("ADDITION_AST\tc\t{:?}", input.width());
}
