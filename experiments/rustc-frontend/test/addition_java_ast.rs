//! Exact primitive Add, original ordered producers and connected typed dataflow.
use super::{AdditionInput, AdditionWidth};
use crate::java_lower::Reader;
use portable_backend_java::ast::*;
#[path = "addition_java_dataflow.rs"]
mod dataflow;
...
pub(super) fn observe<'tcx>(
    ...
    let primitive = match input.width() {
        AdditionWidth::I32 => JavaPrimitive::Int,
        AdditionWidth::I64 => JavaPrimitive::Long,
    };
    ...
    *last = *left.clone();
    ...
    assert_ne!(
        ...
        wanted,
        "commutative value still has ordered source operands"
    );
    eprintln!("ADDITION_DETACHED\tjava");
    eprintln!("ADDITION_AST\tjava\t{:?}", input.width());
}
