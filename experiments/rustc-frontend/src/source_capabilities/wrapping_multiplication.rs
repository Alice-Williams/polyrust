...
pub(crate) struct WrappingMultiplication;
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum MultiplicationWidth {
    I32,
    I64,
}

#[cfg(multiplication_ast_probe)]
#[path = "../../test/multiplication_input_probe.rs"]
mod probe;

pub(crate) struct MultiplicationInput<'tcx> {
    source: &'tcx hir::Expr<'tcx>,
    left: &'tcx hir::Expr<'tcx>,
    right: &'tcx hir::Expr<'tcx>,
    definition: DefId,
    width: MultiplicationWidth,
}
impl Capability for WrappingMultiplication {
    type Input<'tcx> = MultiplicationInput<'tcx>;
}
...
impl<'tcx> MultiplicationInput<'tcx> {
    pub(crate) fn discover(
        ...
        let hir::Node::Expr(canonical) = tcx.hir_node(expression.hir_id) else {
            return Err("wrapping multiplication requires a canonical HIR expression".into());
        };
        if !std::ptr::eq(canonical, expression)
            || !std::ptr::eq(checked, tcx.typeck(expression.hir_id.owner.def_id))
        {
            return Err(
                "wrapping multiplication requires its original checked function and HIR node"
                    .into(),
            );
        }
        let (definition, arguments, left, right) = match canonical.kind {
            hir::ExprKind::MethodCall(_, left, [right], _) => (
                checked
                    ...
                    .ok_or("wrapping multiplication method is unresolved")?,
            ...
            hir::ExprKind::Call(callee, [left, right]) => {
                ...
                let ty::FnDef(typed, arguments) = *checked.expr_ty(callee).kind() else {
                    return Err(
                        "wrapping multiplication requires a direct associated function".into(),
                    );
                };
                if typed != resolved || !checked.expr_adjustments(callee).is_empty() {
                    return Err(
                        "wrapping multiplication associated identity or adjustment differs".into(),
                    );
                }
        ...
        if tcx.item_name(definition).as_str() != "wrapping_mul" {
            return Ok(None);
        }
        let core = tcx
            .lang_items()
            ...
            .ok_or("wrapping multiplication requires the standard core language items")?
            .krate;
        if definition.is_local()
            || definition.krate != core
            || tcx.def_kind(definition) != DefKind::AssocFn
        {
            return Err("only the standard primitive wrapping_mul method is supported".into());
        }
        let implementation = tcx
            .inherent_impl_of_assoc(definition)
            .ok_or("wrapping multiplication requires an inherent primitive implementation")?;
        ...
        {
            return Err(
                "wrapping multiplication requires nongeneric unadjusted by-value operands".into(),
            );
        }
        let owner = tcx
            ...
            .map_err(|_| "wrapping multiplication primitive owner normalization failed")?;
        let width = match owner.kind() {
            ty::Int(ty::IntTy::I32) => MultiplicationWidth::I32,
            ty::Int(ty::IntTy::I64) => MultiplicationWidth::I64,
            _ => return Err("wrapping multiplication supports only exact i32 and i64".into()),
        };
        if [canonical, left, right]
            .into_iter()
            .any(|node| checked.expr_ty(node) != owner)
        {
            return Err("wrapping multiplication operand/result widths differ".into());
        }
        let signature = tcx
            .try_normalize_erasing_regions(
                ty::TypingEnv::fully_monomorphized(),
            ...
            .map_err(|_| "wrapping multiplication signature normalization failed")?;
        if !signature.bound_vars().is_empty() {
            return Err(
                "wrapping multiplication cannot instantiate bound signature variables".into(),
            );
        }
        ...
        {
            return Err(
                "wrapping multiplication requires the exact safe primitive signature".into(),
            );
        }
        Ok(Some(Self {
            source: canonical,
    ...
    pub(crate) fn require_context(
        ...
        let current = Self::discover(tcx, checked, self.source)?
            .ok_or("wrapping multiplication witness no longer names a built-in operation")?;
        ...
        {
            return Err("wrapping multiplication witness identity differs".into());
        }
        Ok(())
    }
    ...
    pub(crate) fn width(&self) -> MultiplicationWidth {
        self.width
    }
}
