"""Three original source owners; exact Rust/C/Java NaN classification and call proof."""
import os
from pathlib import Path
import subprocess
import sys
from nan_inventory import inspect
from nan_oracle import VALUES, OPERATIONS, LITERALS, expected, traces, classification
from nan_consumers import c_consumer, java_consumer
from nan_mutations import mutate
from short_circuit_mutations import instrument
from nan_examples import export
...
def main():
    ...
    work_root = Path(os.environ["TEST_TMPDIR"]) / "nan-native"
    work_root.mkdir()
    ...
    for is_java, directory, owners in [(True, java_dir, java), (False, c_dir, c)]:
        ...
        for variant in ["plain", "traced", "wrong_comparison", "always_false", "dropped", "duplicated"]:
            ...
            for owner in order:
                ...
                if owner == root and variant != "plain":
                    text = instrument(text, {"N": marker}, is_java)
                    if variant != "traced":
                        target = "direct" if variant in ["wrong_comparison", "always_false"] else "local"
            ...
            for result in results:
                ...
                if variant in ["wrong_comparison", "always_false"]:
                    original = list(wanted)
                    for index, bits in enumerate(VALUES):
                        wanted[2 + index * len(OPERATIONS)] = str(
                            1 - classification(bits) if variant == "wrong_comparison" else 0)
                    assert wanted != original
                assert result.stdout.splitlines() == wanted, (is_java, variant, "exact classification oracle")
                assert result.stderr == traces(variant), (is_java, variant, "single operand call")
                if variant in ["dropped", "duplicated"]:
                    assert result.stdout == truth and result.stderr != traces("traced")
    export(java_dir, c_dir, work_root)
    print(f"{len(truth.splitlines())} exact Rust/C/Java NaN classification results; three crates, "
          "GCC/Zig O0/O2, Java21 strict lint; four value/trace faults detected")
